#include "ray.h"
  ...
  float Ray::trace(){
    if(brightness < pow(0.4, 4)){ // Stops a ray when it has already reflected 4 times
      return brightness;
    }
  ...
  Vec3D Ray::reflect(Vec3D normalVec) const{ //  mirrors a directionVec in a vector which is perpendicular to the surface of the object
