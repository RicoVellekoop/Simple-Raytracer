#include "rayscanner.h"
    ...
    float RayScanner::normaliseXValue(int value){ //  returns x coördinate on a scale from -1 to 1
      return ((value-xResolution/2)/(float)xResolution) * 2;
    }

    float RayScanner::normaliseYValue(int value){ //  returns x coördinate on a scale relative to the width of the screen
      return -((value-yResolution/2)/(float)xResolution) * 2;
    }


    void RayScanner::scan(){  //  Prints the image in askii caracters in the console
      static const char askii[8] = {' ', '.', ',', '\"', '/', 'O', '8', '@'};
      for (int y = 0; y < yResolution; y++){  //  these two loops loop trought all pixels
    ...
    void RayScanner::renderImage(float rValue, float gValue, float bValue){
      ...
      */
      ...
      for(int tc = 0; tc < 54; tc++){ //  writes the header to the file
      ...
      for (int y = 0; y < yResolution; y++){
        for (int x = 0; x < xResolution; x++){
          float brightness = Ray(normaliseXValue(x)*fovScaler, -normaliseYValue(y)*fovScaler, 1.0, objects).trace();
          for(int rgb = 0; rgb < 3; rgb++){ // Writes the value 3 times because there are 3 color chanels
            switch(rgb){
              case 0:
