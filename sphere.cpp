#include "sphere.h"
  ...
  std::string Sphere::getType(){  //  Used to be able to know what object a ray has hit
    return "Sphere";
  }
  ...
  bool Sphere::hit(class Ray &ray) const{
    if(this -> hitPoint(ray).distToPoint(ray.getSupVec()) < 0.0001){  //  this is here to make sure the reflected ray wont reflect on the same Sphere
      return false;
    }
    if(this->distFromRay(ray) < radius){
      if(this->hitPoint(ray).sub(ray.getSupVec()).dot(ray.getDirVec()) > 0){  //  Check if Sphere is behind the ray
        return true;
      }else{
        return false;
      }
  ...
  Vec3D Sphere::getNormalVector(class Ray &ray) const{  //  this returns a vector perpendicular to the surface on the sphere where it hits the circle
